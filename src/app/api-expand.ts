...
import { mergeSuggestions, type ExistingNode, type NewNode } from '../features/ai-structuring/merge';

/** Composition of /api/expand (O57): auth → quota → context → AI → add-only suggestions. */
export interface ExpandDeps {
  verify: SessionVerifier;
  loadNodes: (mapId: string, ownerId: string) => Promise<(ExistingNode & { parentId: string | null })[]>;
  isQuotaBlocked: (ownerId: string) => Promise<boolean>;
  chat: ChatFn;
  saveNodes: (mapId: string, ownerId: string, nodes: NewNode[]) => Promise<void>;
...
export function makeExpandHandler(deps: ExpandDeps) {
  return async (req: Request): Promise<Response> => {
    ...
    const parsed = expandInputSchema.safeParse(await req.json().catch(() => ({})));
    if (!parsed.success) return json({ error: 'invalid input' }, 400);
    const { mapId, nodeId, mode } = parsed.data;

    if (await deps.isQuotaBlocked(ownerId)) return json({ error: 'quota_exceeded', upgrade: true }, 402);

    const nodes = await deps.loadNodes(mapId, ownerId);
    const context = mode === 'gaps' ? nodes.map((n) => n.text).join(' / ') : buildNodeContext(nodes, nodeId);
