...
import { createMemoryRateLimiter, makeRateLimited } from "./rate-limit";
...
/**
 * Module-level so the sliding window survives across requests on a warm
 * serverless instance (SEC-004). Cross-instance coverage needs a shared store
 * (Upstash) swapped in at release; the interface (O35) makes that a drop-in.
 */
...
export function serverDeps(
  ...
  const rateLimited = makeRateLimited(sharedRateLimiter, () => now().getTime());

  const assertMapOwner = async (mapId: string, ownerId: string) => {
    const row =
      (await db.select().from(maps).where(eq(maps.id, mapId)).limit(1))[0] ??
      null;
  ...
  return {
    ...
    async saveNodes(mapId: string, ownerId: string, newNodes: NewNode[]) {
      ...
      for (const nn of newNodes) {
        const parentId =
          nn.parentId && tempToId.has(nn.parentId)
            ? tempToId.get(nn.parentId)!
            : nn.parentId;
        const inserted = (
          await db
            .insert(nodes)
    ...
    mapsBackend: {
      ...
      async getById(id) {
        return (
          (await db.select().from(maps).where(eq(maps.id, id)).limit(1))[0] ??
          null
        );
      },
      async insert(ownerId, title) {
        return (
          await db.insert(maps).values({ ownerId, title }).returning()
        )[0];
      },
