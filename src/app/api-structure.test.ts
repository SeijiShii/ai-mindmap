...
const chatOk: ChatFn = async () => ({
  content: JSON.stringify({
    suggestions: [
      { tempId: "t1", parentRef: null, text: "新しい要点", kind: "tree" },
    ],
  }),
...
describe("makeStructureHandler (O57 composition / P4.46 / SEC-004)", () => {
  ...
  it("400 on invalid input (SEC-002 boundary)", async () => {
    ...
    const res = await h(
      reqWith({ mapId: "not-a-uuid", transcriptDelta: "hi" }),
    );
  ...
  it("AS-S1: 200 adds merged nodes + records usage", async () => {
    ...
    const h = makeStructureHandler(
      deps({ saveNodes: save, recordUsage: record }),
    );
    const res = await h(
      reqWith({ mapId: UUID, transcriptDelta: "会議の要点" }),
    );
