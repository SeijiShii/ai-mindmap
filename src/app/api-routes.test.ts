...
function mapsBackend(rows: MapRow[] = []): MapBackend {
  ...
  return {
    async listByOwner(o) {
      return [...m.values()].filter((r) => r.ownerId === o);
    },
    async getById(id) {
      return m.get(id) ?? null;
    },
    async insert(o, t) {
      const r = { id: `m${++seq}`, ownerId: o, title: t };
      m.set(r.id, r);
      return r;
    },
    async rename() {},
...
const chatExpand: ChatFn = async () => ({
  content: JSON.stringify({
    suggestions: [
      { tempId: "t", parentRef: null, text: "関連案", kind: "relation" },
    ],
  }),
      ...
      (
        await h(
          new Request("https://x/api/expand", {
            method: "POST",
            body: JSON.stringify({ mapId: UUID, nodeId: UUID }),
          }),
        )
      ).status,
    ).toBe(401);
